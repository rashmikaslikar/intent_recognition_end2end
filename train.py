...
app = typer.Typer()
experiment_name=f"bert-{int(time.time())}"
dataset_loc = os.path.join(Path.home(),'intent_recognition/datasets','search_queries_candidate_dataset__jan_2024.csv')
...
def train_step(
    ...
    model.train()
    loss = 0.0
    ds_generator = ds.iter_torch_batches(batch_size=batch_size, collate_fn=utils.collate_fn, prefetch_batches=10)
    for i, batch in enumerate(ds_generator):
        ids, masks = batch["ids"], batch["masks"]
        optimizer.zero_grad()  # reset gradients
        z = model(ids,masks)  # forward pass
...
def train_loop_per_worker(config: dict) -> None:  # pragma: no cover, tested via train workload
    ...
    for name, param in bert.named_parameters():
        if not any(n in name for n in ['encoder.layer.11', 'encoder.layer.10', 'encoder.layer.9']):
            #print(name)
            param.requires_grad = False

    # pass the pre-trained BERT to our defined model architecture
    model = BERT_Arch(bert,num_classes).to('cpu')
    #model = train.torch.prepare_model(model,'cpu')
...
@app.command()
def train_model(
    experiment_name: Annotated[str, typer.Option(help="name of the experiment for this training workload.")] = experiment_name,
    dataset_loc: Annotated[str, typer.Option(help="location of the dataset.")] = dataset_loc,
    train_loop_config: Annotated[str, typer.Option(help="arguments to use for training.")] = train_loop_config,
    ...
    train_ds, val_ds = data.stratify_split(ds, stratify="intent", test_size=0.2)
    tags = train_ds.unique(column="intent")
    ...
    train_ds = preprocessor.transform(train_ds,'train')
    val_ds = preprocessor.transform(val_ds, 'val')
    ...
    trainer = TorchTrainer(
        ...
        metadata={"intent_to_index": preprocessor.intent_to_index},
    )
